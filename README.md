...
1. **Request a-Shell to bundle PyNaCl** — we've filed [holzschu/a-shell#1027](https://github.com/holzschu/a-shell/issues/1027) requesting that PyNaCl be included as a pre-compiled framework. If accepted, it would work for all a-Shell users without any extra steps.
2. **Build a custom a-Shell** — sign both the app and the framework with your own Apple Developer identity (same Team ID → `dlopen()` succeeds). See `IOS_CODESIGN_GUIDE.md` for full instructions.
3. **Jailbroken device** — `ldid -S` pseudo-signatures work when kernel enforcement is disabled
4. **Use pure-Python fallback** — avoid native extensions entirely (not possible for PyNaCl)
